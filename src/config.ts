...
export function removeModule(modulePath: string): Config {
  ...
  config.myModules = config.myModules.filter((m: string) => m !== normalized);
...
export function isMyPR(changedFiles: string[], myModules: string[]): boolean {
  if (myModules.length === 0) return true; // no filter → include all
  return changedFiles.some((file) =>
    myModules.some((mod) => file.startsWith(mod + "/") || file === mod),
  );
}
